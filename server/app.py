...
def calculate():
    ...
    # Call get_nearby_places to fetch nearby places for the central point
    central_point = None
    if closest_points:
        ...
        nearby_places = get_nearby_places(central_point)
    else:
        nearby_places = []
    ...
    return jsonify({
        ...
        "central_point": central_point,  # Midpoint of the closest points
        "nearby_places": nearby_places  # List of nearby places
    })
...
def get_nearby_places(central_point):
    if not central_point or len(central_point) != 2:
        print("Invalid central_point:", central_point)
        return []

    url = "https://www.onemap.gov.sg/api/public/revgeocode"
    params = {
        "location": f"{central_point[0]},{central_point[1]}",  # Latitude,Longitude
    ...
    try:
        ...
        print("Exception occurred while fetching nearby places:", str(e))
